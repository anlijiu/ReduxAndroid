    ...
    public void update(List<T> newItems) {
        ...
        Observable.create(new ObservableOnSubscribe<DiffUtil.DiffResult>() {
            ...
            public void subscribe(ObservableEmitter<DiffUtil.DiffResult> observableEmitter) throws Exception {
                Timber.d("ssssss before doCalculateDiff %s", Instant.now().toString());
                DiffUtil.DiffResult result = doCalculateDiff(list, newItems);
                Timber.d("ssssss after doCalculateDiff %s", Instant.now().toString());
                observableEmitter.onNext(result);
                observableEmitter.onComplete();
            }
        }).compose(RxUtils.observableToMainTransformer()).subscribe(result -> dispatchUpdates(newItems, result));
//        list = newItems;
//        diffResult.dispatchUpdatesTo(listCallback);
    }

    private void dispatchUpdates(List<T> newItems, DiffUtil.DiffResult result) {
        Timber.d("ssssss before dispatchUpdates %s", Instant.now().toString());
        list = newItems;
        result.dispatchUpdatesTo(listCallback);
        Timber.d("ssssss after dispatchUpdates %s, list size is %d", Instant.now().toString(), list.size());
    }
